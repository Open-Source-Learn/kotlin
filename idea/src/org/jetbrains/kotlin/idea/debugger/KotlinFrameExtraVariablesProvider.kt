...
private fun findAdditionalExpressions(position: SourcePosition): Set<TextWithImports> {
    ...
    while (startLine > limit.getStartOffset() && shouldSkipLine(file, doc, startLine)) {
        startLine--
    }

    var endLine = Math.min(line + 2, limit.getEndOffset())
    while (endLine < limit.getEndOffset() && shouldSkipLine(file, doc, endLine)) {
        endLine++
    }
...
private fun getContainingElement(element: PsiElement): PsiElement? {
    val contElement = PsiTreeUtil.getParentOfType(element, javaClass<JetDeclaration>()) ?: PsiTreeUtil.getParentOfType(element, javaClass())
...
private class VariablesCollector(
    ...
    private fun isRefToProperty(expression: JetReferenceExpression): Boolean {
        ...
        if (descriptor is PropertyDescriptor) {
            return descriptor.getCompileTimeInitializer() == null
        }
        return false
    }
