...
public class JetFunctionCallUsage extends JetUsageInfo<JetCallElement> {
    ...
    private void updateArgumentsAndReceiver(JetChangeInfo changeInfo, JetCallElement element) {
        ...
        List<JetParameterInfo> newSignatureParameters = changeInfo.getNewSignatureParameters();
        ...
        if (newReceiverInfo != null) {
            ...
            JetExpression dispatchReceiverExpression = getReceiverExpression(dispatchReceiver, psiFactory);
            if (elementToReplace instanceof JetQualifiedExpression && dispatchReceiverExpression != null) {
                String callText = "with(" + dispatchReceiverExpression.getText() + ") {\n " + receiverText + "." + element.getText() + " \n}";
                replacingElement = psiFactory.createExpression(callText);
            }
            else {
