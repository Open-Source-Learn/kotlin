...
public abstract class JetExplicitParameterReferenceUsage<T: JetElement>(element: T) : JetUsageInfo<T>(element) {
...
) : JetExplicitParameterReferenceUsage<JetSimpleNameExpression>(element) {
    override fun processReplacedElement(element: JetElement) {
        element.addToShorteningWaitSet(ShorteningOptions(removeThis = true))
    }
...
) : JetExplicitParameterReferenceUsage<JetThisExpression>(element) {
    override fun processReplacedElement(element: JetElement) {
        element.addToShorteningWaitSet(ShorteningOptions(removeThisLabels = true))
    }
