...
public abstract class AbstractKotlinEvaluateExpressionTest : KotlinDebuggerTestBase() {
    ...
    fun getExtraVars(): Set<TextWithImports> {
        DebuggerSettings.getInstance().AUTO_VARIABLES_MODE = true;
        val method = javaClass<JavaStackFrame>().getDeclaredMethod("computeExtraVars",
                                                                   javaClass<com.intellij.openapi.util.Pair<*, *>>(),
                                                                   javaClass<SourcePosition>(),
                                                                   javaClass<EvaluationContextImpl>())
        method.setAccessible(true)

        val vars: MutableSet<String> = hashSetOf()
        val extras: MutableSet<TextWithImports> = hashSetOf()

        [suppress("UNCHECKED_CAST")]
        return method.invoke(null, com.intellij.openapi.util.Pair(vars, extras), debuggerContext.getSourcePosition(), evaluationContext) as Set<TextWithImports>
    }
